...
%\begin{observation}
%  \label{obs:product-vs-single}
%  In the previous section we made use of the observables (\ref{eq:xyy-observables}) to predict the result of a spin component measurement of one particle by measuring first suitable spin components of the other two particles. Such measurements on the first two particles inevitably project state (\ref{eq:ghz-state}) into a factorized state. Yet, we stated in this section that the state (\ref{eq:ghz-state}) is an eigenstate of all observables (\ref{eq:xyy-observables}). Thus, a measurement of any of the observables (\ref{eq:xyy-observables}) shouldn't change the state of the system.
%
%  This apparent conflict is resolved by observing that the formalism doesn't take care of the collapse of the wavefunction. In the previous section we COLLAPSE IN THREE STEPS...
%\end{observation}
